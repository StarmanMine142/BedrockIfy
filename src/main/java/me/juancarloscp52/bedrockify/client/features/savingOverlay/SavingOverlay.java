...
//            RenderSystem.setShaderColor(1,1,1,BedrockifyClient.getInstance().hudOpacity.getHudOpacity(false));
            // Draw chest
            drawContext.drawTexture(RenderPipelines.GUI_TEXTURED, WIDGET_TEXTURE, client.getWindow().getScaledWidth()-(21+settings.getScreenSafeArea()), 19 + settings.getScreenSafeArea(), 0, 99, 16, 17, 256, 256);
            // Draw arrow
            renderTimer+= BedrockifyClient.getInstance().deltaTime*0.000000001f;
            drawContext.drawTexture(RenderPipelines.GUI_TEXTURED, WIDGET_TEXTURE, client.getWindow().getScaledWidth()-(19+settings.getScreenSafeArea()), 5 + settings.getScreenSafeArea() + MathHelper.floor(MathHelper.abs(MathHelper.sin(renderTimer * 3.1415926F) * 6)), 16, 100, 12, 15, 256, 256);
